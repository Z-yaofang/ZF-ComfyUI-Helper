...
用 `路线数量` 动态显示 1—32 路文本输入，节点中的数字按钮保持单路选择。点击一路后其余路线自动取消选择；减少路线导致原选择失效时，优先切到仍有连接的第一路。执行时若选中的数字路线为空，节点会从上到下返回第一条非空文本。独立的 `空文本` 按钮会直接输出 `""` 和路线索引 `0`，不会触发回退；`实际路线` 输出最终采用的索引。
